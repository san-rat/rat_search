...
This guide covers the v0.1 local run, Linux packaging path, and startup
...
- `Alt+Space` toggles the launcher.
- Search results use the in-memory app catalog scanned at startup.
...
Version 0.1 builds Debian and RPM packages by default:
...
the user presses the launcher hotkey. Version 0.1 prepares for that workflow but
...
The main startup cost in v0.1 is scanning installed `.desktop` application
entries once and keeping normalized app records in memory. Search uses that
in-memory catalog, so typing should stay responsive and should not trigger
repeated filesystem scans.

The v0.1 acceptance target is that Rat Search remains comfortable on an Ubuntu
